## Hi, I'm Sadiya 👋

🎓 PhD researcher in NLP, GenAI, and Machine Learning  
🧠 Focused on mental health NLP, multilingual evaluation, and LLM alignment  
💻 Python | PyTorch | HuggingFace | NLP | LLM Evaluation

### 🔍 Research Interests
- Mental health text analysis
- Multilingual & low-resource NLP
- LLM evaluation, prompting, and alignment
- Dataset creation & benchmarking

### 📌 Featured Projects
(see pinned repositories below 👇)

### 📄 Publications & Research
- Coming soon

### 🤝 Let’s Connect
- GitHub: https://github.com/SadiyaPuspo
- LinkedIn: [(add link)](https://www.linkedin.com/in/sadiya-puspo/)
- Google Scholar: (https://scholar.google.com/citations?hl=en&user=-BDt9hoAAAAJ&view_op=list_works&sortby=pubdate)
